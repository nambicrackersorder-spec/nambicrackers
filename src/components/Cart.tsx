...
export function Cart({ lines, setQty, clear, onClose, onDone }: Props) {
  ...
  const minOrderValue = settings.minOrder ?? SHOP.minOrder;
  const belowMin = netTotal < minOrderValue;
  ...
  const validate = () => {
    ...
    if (belowMin) return `Minimum order value is Rs ${minOrderValue.toLocaleString("en-IN")}.`;
    return "";
  };
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          <div ref={formRef} className="mt-5 space-y-3">
            ...
            <p className="text-center text-xs font-semibold text-muted-foreground">
              Minimum Order Value ₹{minOrderValue.toLocaleString("en-IN")}
            </p>
        ...
        <div className="border-t border-border bg-background px-4 py-3 shadow-[0_-6px_20px_-12px_rgba(0,0,0,0.3)]">
          ...
          >
            {sending
              ? "Placing Order..."
              : belowMin
                ? `Place order Min.₹${minOrderValue.toLocaleString("en-IN")}`
                : "Place Order"}
          </button>
        </div>
