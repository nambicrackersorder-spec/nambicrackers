...
import { CheckCircle2, Circle, Search, Truck, PackageCheck, CreditCard } from "lucide-react";
...
const STATUS_STYLES: Record<string, string> = {
  ...
  "Payment Completed": "bg-indigo-100 text-indigo-800",
...
function TrackPage() {
  ...
  const search = async (e: React.FormEvent) => {
    ...
    setResults(null);
    try {
      const res = await fetch(`${APPS_SCRIPT_URL}?action=track&query=${encodeURIComponent(q)}`);
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <span
                      className={`mt-2 inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_STYLES[o.status || "Confirmed"] || STATUS_STYLES.Confirmed}`}
                    >
                      {o.status || "Confirmed"}
                    </span>
                  </div>
