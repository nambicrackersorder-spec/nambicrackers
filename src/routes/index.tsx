...
function Index() {
  const { activeCategories, activeProducts } = useCatalog();
  ...
  const lines: CartLine[] = useMemo(
    () =>
      activeProducts.filter((p) => (qty[p.id] ?? 0) > 0).map((p) => ({ ...p, qty: qty[p.id] ?? 0 })),
    [activeProducts, qty],
  );
  ...
  const categories = q
    ? activeCategories.map((c) => ({
        ...
        products: c.products.filter(
          (p) => p.name.toLowerCase().includes(q) || (p.tamil && p.tamil.includes(query.trim())),
        ),
      ...
      <section className="border-b border-border bg-secondary">
        <div className="mx-auto max-w-5xl px-4 py-7 text-center">
          ...
          <h1 className="mt-3 text-2xl font-bold text-primary sm:text-3xl">
            Crackers Discount at {settings.discount}%
          </h1>
          <p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">{settings.address}</p>
          <p className="mt-3 inline-block rounded-full bg-primary px-4 py-1.5 text-xs font-semibold text-primary-foreground">
            Minimum order Rs {settings.minOrder.toLocaleString("en-IN")}
          </p>
        </div>
      </section>
