...
function About() {
  ...
  const faqJson = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: `Does ${settings.name} deliver outside Tamil Nadu?`,
        acceptedAnswer: {
          "@type": "Answer",
          text: `Yes. ${settings.name} is based in Sivakasi and delivers across India for confirmed orders, with safe packing and order tracking for each shipment.`,
        },
      },
      {
        "@type": "Question",
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text: `The minimum order value is ₹${settings.minOrder.toLocaleString("en-IN")}. Orders are handled as direct enquiry confirmation and billing through the shop.`,
        },
      },
      {
        "@type": "Question",
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text: `${settings.name} offers up to ${settings.discount}% off MRP on our Sivakasi firework price list.`,
        },
      },
    ],
  };
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <p className="mt-4 text-base leading-relaxed text-foreground">
          {settings.name} is based in Sivakasi, the home of Indian fireworks, and we now deliver
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {[
            {
              t: "Best Price",
              d: `Direct factory rates with ${settings.discount}% off MRP - no hidden charges.`,
            },
            {
              t: "Safe Packing",
              d: "Every parcel is checked by hand and packed for a safe journey.",
            },
            {
              t: "Fast Delivery",
              d: "Pan-India delivery with order tracking and confirmed dispatch.",
            },
          ].map((c) => (
          ...
          <div className="mt-4 space-y-4 text-sm text-muted-foreground">
            <div>
              <h3 className="font-semibold text-foreground">
                Does {settings.name} deliver outside Tamil Nadu?
              </h3>
              <p className="mt-1">
                Yes. {settings.name} is based in Sivakasi and delivers across India for confirmed
                orders, with safe packing and order tracking.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-foreground">What is the minimum order value?</h3>
              <p className="mt-1">
                The minimum order value is ₹{settings.minOrder.toLocaleString("en-IN")}. Orders are handled as direct enquiry confirmation
                and billing through the shop.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-foreground">How much discount do you offer?</h3>
              <p className="mt-1">
                {settings.name} offers up to {settings.discount}% off MRP on our Sivakasi firework price list.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 rounded-xl bg-secondary p-5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {settings.phoneDisplay} &middot; {settings.email}
          </p>
        </div>
      </main>
      <SiteFooter />
